...
void fv_epoll_add(int epfd, int fd, struct epoll_event *event)
{
    int rc = epoll_ctl(epfd, EPOLL_CTL_ADD, fd, event);
    check(rc == 0, "epoll_add");
}

void fv_epoll_mod(int epfd, int fd, struct epoll_event *event)
{
    int rc = epoll_ctl(epfd, EPOLL_CTL_MOD, fd, event);
    check(rc == 0, "epoll_mod");
}

void fv_epoll_del(int epfd, int fd, struct epoll_event *event)
{
